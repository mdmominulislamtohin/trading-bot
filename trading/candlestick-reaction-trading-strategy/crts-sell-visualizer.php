...
<head>
    ...
    <title>CRTS Bearish (Sell) Monitor | ADA 3m</title>
    ...
    <style>
        ...
        .container { max-width: 1350px; margin: 0 auto; padding: 20px; } /* Slightly wider for 8 cards */
        ...
        .pulse-down { background: var(--down); }
        ...
        .p-entry { color: var(--warn); font-weight: 700; }
        ...
        .analysis-box { background: rgba(255,255,255,0.02); padding: 10px; border-radius: 8px; font-size: 0.8rem; border-left: 3px solid var(--down); line-height: 1.5; }
...
<div class="container">
    <div class="header-stats">
        <div class="ticker-card">
            ...
            <span class="t-val" id="cur-price" style="color: var(--down);">0.00000</span>
        </div>
        <div class="ticker-card">
        ...
        <div class="ticker-card">
            <span class="t-label">Active / Total (Short Signals)</span>
    ...
    <div class="section-title">
        ...
        <b>Multi-Ratio Simulation (Short 1000 ADA Trade Size)</b>
    </div>
...
<script>
    ...
    // 🔥 UPDATED: Added 1, 3, and 4 to the RR targets
    ...
    async function updateData() {
        try {
            ...
            if (data.status === 'success' && data.candles) {
                ...
                runBearishSimulation(data.candles);
            }
        } catch(e) { console.error("Sync error", e); }
    }

    function runBearishSimulation(rawData) {
        ...
        for (let i = 1; i < data.length - 1; i++) {
            ...
            if (c1.close <= c1.open && c2.close <= c2.open && c2.high < c1.high) {
                ...
                for (let j = i + 1; j < data.length; j++) {
                    ...
                    if (!isDead) {
                        RR_TARGETS.forEach(rr => { 
                            if (trade.rrResults[rr].status === 'Running' && f.low <= trade.rrResults[rr].tp) { 
                                trade.rrResults[rr].status = 'Closed'; 
    ...
    function renderUI(trades, stats) {
        ...
        trades.forEach(t => {
            ...
            let levelsHtml = `<span class="price-tag p-entry">SHORT: ${t.entryPrice.toFixed(5)}</span><span class="price-tag p-sl">SL: ${t.sl.toFixed(5)}</span>`;
            ...
            logHtml += `
                <tr class="${isOverallRunning ? 'is-running' : ''}">
                    ...
                    <td><div class="analysis-box">Max Drop reached: <b>+${maxGainPts} Pts</b>. Market Low: $${t.minPrice.toFixed(5)}</div></td>
                </tr>`;
        });
        document.getElementById('log-body').innerHTML = logHtml || '<tr><td colspan="5" style="text-align:center;">No bearish patterns found yet.</td></tr>';
