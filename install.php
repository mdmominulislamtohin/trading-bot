...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($_POST['csrf_token']) || $_POST['csrf_token'] !== ($_SESSION['csrf_token'] ?? '')) {
        ...
        $bsc_key = trim($_POST['BSCSCAN_API_KEY'] ?? '');
        $alchemy_key = trim($_POST['ALCHEMY_API_KEY'] ?? '');
        ...
        if (!$wrote) {
            $errors[] = 'Failed to write .env file to project root. Please create the file manually with the following contents (shown below).';
        }
        ...
        if ($pdo && empty($errors)) {
            // create users table if not exists (migration might have created it)
            try {
...
<form method="post">
  ...
  <label>DB Name
    <input type="text" name="DB_NAME" value="trading" required>
  </label>
  <label>DB User
  ...
  <label>BscScan API Key
    <input type="text" name="BSCSCAN_API_KEY" value="">
  </label>
  <label>Alchemy API Key (Arbitrum)
    <input type="text" name="ALCHEMY_API_KEY" value="">
  </label>
  ...
  <label>Admin email
    <input type="text" name="ADMIN_EMAIL" value="admin@example.com" required>
  </label>
  ...
  <label>APP_MASTER_KEY (optional — leave empty to auto-generate)
    <input type="text" name="APP_MASTER_KEY" placeholder="Leave empty to auto-generate">
  </label>

  <label><input type="checkbox" name="RUN_MIGRATIONS" checked> Run DB migrations now</label>
  <label><input type="checkbox" name="GENERATE_TOTP"> Generate Admin TOTP secret now (recommended)</label>

  <div style="margin-top:12px"><button type="submit">Install</button></div>
</form>
...
<?php if ($showResult): ?>
  ...
  <?php else: ?>
    <p style="color:red">Installation completed with errors:</p>
    <ul>
      <?php foreach ($errors as $e): ?>
        <li><?php echo htmlspecialchars($e) ?></li>
      <?php endforeach; ?>
    </ul>
    <p>If .env was not written, create it manually with the contents below.</p>
  <?php endif; ?>
